...
#if defined(NDEBUG)
# error "Beancoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd606fe90f87b6d28537bc43367488f6f08695e54c46538bed384d528eac1e43d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Beancoin: starting difficulty is 1 / 2^12
...
{
    // Beancoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Beancoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Beancoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Beancoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Beancoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Beancoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5395d4ca66e9a5cbf63d7deed075d93898a93cb3674f9bfe91162ab61119798d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sarah Sanders apoligizes for false claim about African-American jobs";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c1efbf787fc451e8d3ec3c263892ea642945fc3b09952007fb459c37553fe56a2cfd7f286f53c8c5ffa75fd902b4cc7f813253e29f722aa9eb292af9c19f98d1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534354250;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085261990;

        if (fTestNet)
        {
            block.nTime    = 1534354227;
            block.nNonce   = 386029022;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xae5ec3df709416fa9c750ba2c531dc97eca6763c33fbde55fe66c47ed0233498"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Beancoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeancoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeancoinMiner(CWallet *pwallet)
{
    printf("BeancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beancoin-miner");
    ...
    try { loop {
        ...
        printf("Running BeancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BeancoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeancoinMiner, pwallet));
}
